 ...
 */
const enrichRecordsWithStatus = async (
    records: any[]
    ...
    return records.map((record) => ({
        ...record,
        status:
            record.status_id && statusMapping[record.status_id]
                ? {
                      id: record.status_id,
                      wartosc: statusMapping[record.status_id],
                  }
                : undefined,
    }));
};
...
export const getAttendanceByStudent = async (
    ...
    try {
        const res = await fetch(`${BASE}/api/frekwencja/?uczen_id=${uczenId}`, {
            headers: headers(),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
...
/**
 * GET /api/frekwencja/?data=YYYY-MM-DD&uczen_id=X - Pobierz frekwencję dla konkretnej daty i ucznia
 * Falls back to fetching by student and filtering by date if needed
 */
export const getAttendanceByDate = async (
    ...
    try {
        // Try with both parameters (correct URL format with &)
        const res = await fetch(
            `${BASE}/api/frekwencja/?data=${date}&uczen_id=${uczenID}`,
            { headers: headers() }
        );
        ...
        // Fallback: fetch by student ID only (faster than fetching ALL) and filter by date
        console.warn(
            "[attendance] Combined filter failed, falling back to student filter"
        );
        const studentRecords = await getAttendanceByStudent(uczenID);
        return studentRecords.filter((record) => record.data === date);
    } catch (e) {
        console.error("[attendance] getAttendanceByDate error:", e);
        // Fallback: fetch by student and filter by date
        try {
            const studentRecords = await getAttendanceByStudent(uczenID);
            return studentRecords.filter((record) => record.data === date);
        } catch {
